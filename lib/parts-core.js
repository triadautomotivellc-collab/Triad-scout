import { canonicalText, numberFrom, parseVehicleQuery } from './scout-core.js';

export const PARTS_ENGINE_VERSION = '0.8.2-alpha';
...
const PART_QUERY_STRIP_RX = [
  /\b(?:left|right|lh|rh|driver|passenger)\b/gi,
  /\b(?:front|rear)\b/gi,
  /\b(?:6l80e?|6l90e?|4l60e?|10r80)\b/gi,
  /\b(?:transmission|trans|gearbox|engine|motor|long block|short block)\b/gi,
  /\b(?:turbocharger|turbo|headlight|headlamp|taillight|tail light|tail lamp)\b/gi,
  /\b(?:door assembly|front door|rear door|door|hood|bonnet|fender)\b/gi,
  /\b(?:bumper cover|bumper|radiator support|core support|radiator)\b/gi,
  /\b(?:condenser|ac condenser|a\/c condenser|alternator|starter|transfer case)\b/gi,
  /\b(?:rear differential|differential|rear end)\b/gi,
  /\b(?:used|salvage|replacement|part|parts|oem)\b/gi
];

export function inferVehicleFromPartText(text='', fallback={}){
  const raw=String(text||'').trim();
  let cleaned=raw;
  for(const rx of PART_QUERY_STRIP_RX) cleaned=cleaned.replace(rx,' ');
  cleaned=cleaned.replace(/\s+/g,' ').trim();
  const parsed=parseVehicleQuery(cleaned);
  const engineMatch=raw.match(/\b(\d\.\d)\s*[lL]\b/) || raw.match(/\b(\d\.\d)\b/);
  const inferred={
    year:parsed.years?.[0]||null,
    make:parsed.make||'',
    model:parsed.model||'',
    trim:parsed.trims?.[0]||'',
    engine:engineMatch?`${engineMatch[1]}L`:'',
    transmission:'',
    vin:''
  };
  return {
    year:Number(fallback.year)||inferred.year,
    make:String(fallback.make||inferred.make||''),
    model:String(fallback.model||inferred.model||''),
    trim:String(fallback.trim||inferred.trim||''),
    engine:String(fallback.engine||inferred.engine||''),
    transmission:String(fallback.transmission||inferred.transmission||''),
    vin:String(fallback.vin||'')
  };
}

export function donorVehicleSearchText(text='', fallback={}){
  const v=inferVehicleFromPartText(text,fallback);
...
export function parsePartQuery(text='', vehicleContext={}){
  ...
  const matchedAlias=PART_ALIASES.find(a=>a.rx.test(raw))||null;
  const partType=matchedAlias?.canonical||'';
  const variantTokens=matchedAlias?[...matchedAlias.tokens]:[];
  let side=null;
  ...
  return {
    raw,
    partType,
    recognizedPart:Boolean(matchedAlias),
    intent:matchedAlias?'part':'vehicle_or_unknown',
    variantTokens,
    side,
    oemPartNumber:oem,
    interchangeNumber:interchange,
    vehicle:inferVehicleFromPartText(raw,vehicleContext)
  };
}
...
export function resolveInterchange(query, records=[]){
  if(!query?.partType) return {resolved:false,partType:'',groupIds:[],members:[],compatibleVehicles:[],confidence:0,source:[]};
